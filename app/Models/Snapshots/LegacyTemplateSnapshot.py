"""
    subclass specific to Legacy Index Templates
"""
import os
import json
import logging
...
class LegacyIndexTemplateSnapshot(Snapshot):
    """
        Legacy Index Template specific snapshot
    """
    ...
    def output_to_file(self, payload) -> None:
        ...
        for index_template in payload:
            ...
            try:
                with open(os.path.join(path, index_template + ".json"),'x') as file:
                    file.write(json.dumps({index_template: payload[index_template]}, indent=2))
